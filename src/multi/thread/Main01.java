...
public class Main01 {
    public static void main(String[] args) {
        ...
        //스레드 - Thread(run메소드,start메소드,,,,)
        //start >> 실행대기(runnable) >> 실행(run)
        // >> sleep >> 일시정지 >> 인터럽트(interrupt)
        // >> 실행대기(runnable) >> run >> 반복,,,,
        //stop : 스레드 즉시종료 - 불안전 종료를 유발하므로 사용하지않는 것 권장
        //인터럽트 : 일시정지 상태의 스레드에서 InterruptedException발생시켜서
        //          catch문에서 실행대기상태로 가거나 종료상태로 갈 수있도록 해준다.
        ...
        //3.이미 다른클래스를 상속받은 상황-Thread를 상속받을 수없다.
        //이런한 경우 Runnable인터페이스를 상속받고 구현가능하다.
